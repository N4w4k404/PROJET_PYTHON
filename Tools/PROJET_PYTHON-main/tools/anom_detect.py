...
def check_warning() :
    ...
    res_check_ip = check_ip(cnt)
    print("CHECK_IP : ",res_check_ip)
    # Détecte les potentiel flood de requête syn
    res_detect_syn_flood = detect_syn_flood(packet)
    print("DETECT SYN FLOOD : ",res_detect_syn_flood)
    # Détecte les arp spoofing
    print(f"\n----------------------- Détection d'arp spoofing")
    res_detect_arp_spoofing =  detect_arp_spoofing(packet)
    print("DETECT ARP SPOOFING",res_detect_arp_spoofing)

    print(f"\n----------------------- Détection de présences malveillante dans les packet")
    # Lit les paquets et détecte 'malicious'
    res_detect_malicious_payload = detect_malicious_payload(packet)
    print("DETECT MALICIOUS PAYLOAD :", res_detect_malicious_payload)
    ...
    return {
        "CHECK_IP": res_check_ip,
        "DETECT_SYN_FLOOD": res_detect_syn_flood,
        "DETECT_ARP_SPOOFING": res_detect_arp_spoofing,
        "DETECT_MALICIOUS_PAYLOAD": res_detect_malicious_payload
    }
